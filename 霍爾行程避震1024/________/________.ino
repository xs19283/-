...
void PutValueArray(float Array[]) {
  for (int i = 0; i < NUMBER; i++) {
    Array[i] = scale.bias_read();
  }
  HallStartValue = scale.bias_read();
}
...
void CalculateKG(int *SendHx711) {
  *SendHx711 = (Hx711SortArray[2] / 33000) * 10;
}
...
//////////////////////距離計算涵式//////////////////////
void CalDistance(int *SendDis) {
  if (SendHx711 >= 16 && SendHall >= 24) {
    *SendDis = 4;
  } else if (SendHx711 >= 10 && SendHall >= 24) {
    *SendDis = 3;
  } else if (SendHx711 >= 7 && SendHall >= 10) {
    *SendDis = 2;
  } else if (SendHx711 >= 4 && SendHall >= 4) {
    *SendDis = 1;
  } else if (SendHx711 >= 0 && SendHall >= 0) {
    *SendDis = 0;
  }
}
...
void NowModeSwitch() {
  ...
  if (HILO == HIGH) {
    InteHall();
  } else  if (SendZ >= 100 && CtrlInti == 0 && SendHx711 <= 10) {
    digitalWrite(LED, HIGH);
    MotorCmd(6);
    NowMode = 4;
    delay(1500);
    ...
    MotorCmd(1);
    NowMode = 1;
  } else if (abs(SendHx711) >= 10 && CtrlInti == 0) {
    digitalWrite(LED, HIGH);
    switch (Seg) {
      case 1:
        if (SendZ >= 25) {
